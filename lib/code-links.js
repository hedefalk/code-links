...
const matchScope = function(scopeName) {
    return function(providerAndMapper) {
        return (providerAndMapper.provider.scopes.indexOf(scopeName) !== -1);
    };
};
...
const editorToTextContent = function(e) {
    return e.getText();
};
...
module.exports = {
    ...
    consumeLinkProviderInternal(provider, editorMapper) {
        ...
        var providerAndMapper = {provider: provider, mapper: editorMapper};

        this.processors.push(providerAndMapper);
        this.onProvidersChanged();
        return new Disposable(() => {
            let idx = this.processors.indexOf(providerAndMapper);
            if (idx > -1) {
    ...
    consumeLinkProviderV0(provider) {
        return this.consumeLinkProviderInternal(provider, editorToTextContent);
    },
    consumeLinkProviderV1(provider) {
        return this.consumeLinkProviderInternal(provider, identity);
    },
    onProvidersChanged() {
    ...
    processEditor(editor) {
        try {
            ...
            links = this.processors
                ...
                .map(function(providerAndMapper) {
                    return providerAndMapper.provider.process(providerAndMapper.mapper(editor)).map(function(link) {
        ...
        if (markers.length) {
            ...
            if (filename) {
                ...
                atom.workspace.open(filename).then((editor) => {
                    ...
                    let source = editor.getText();
                    let dest = provider.scanForDestination(editor, marker);
